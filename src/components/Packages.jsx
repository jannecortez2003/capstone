      ...
      <div className="container mx-auto px-4 max-w-7xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-stretch">
          {packages.map((pkg, index) => (
            <div key={index} className={`bg-white dark:bg-gray-800 rounded-2xl shadow-sm border ${pkg.popular ? 'border-pink-400 shadow-pink-100 dark:shadow-none relative' : 'border-pink-100 dark:border-gray-700'} p-8 hover:shadow-md transition-all duration-300 flex flex-col h-full`}>
              {pkg.popular && (
              ...
              <button 
                ...
                className={`w-full py-3 rounded-xl font-bold transition shadow-sm ${pkg.popular ? 'bg-pink-600 text-white hover:bg-pink-700' : 'bg-pink-50 dark:bg-gray-700 text-pink-600 dark:text-pink-400 hover:bg-pink-100 dark:hover:bg-gray-600'}`}
              >
                Select Package
              </button>
            </div>
